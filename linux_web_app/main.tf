...
resource "azurerm_linux_web_app" "app_service_linux" {
    ...
    content {
      ...
      dynamic "application_stack" {
        ...
        content {
          ...
          # dotnet_version               = lookup(local.site_config.application_stack, "dotnet_version", "7.0")
          ...
          node_version                 = lookup(local.site_config.application_stack, "node_version", "18-lts")
          # php_version                  = lookup(local.site_config.application_stack, "php_version", 8.2)
  ...
  dynamic "backup" {
    ...
    content {
      ...
      storage_account_url = local.storage_account_container_uri
  ...
  dynamic "storage_account" {
    for_each = length(var.mount_points) > 0 ? [] : var.mount_points
    iterator = mp
    content {
...
#   resource_group_name = var.resource_group_name
#   location            = var.location
#   pfx_blob            = lookup(each.value, "certificate_file", null) != null ? filebase64(each.value.certificate_file) : null
#   password            = lookup(each.value, "certificate_password", null)
#   key_vault_secret_id = lookup(each.value, "certificate_keyvault_certificate_id", null)
# }
...
#   hostname            = each.key
#   app_service_name    = azurerm_windows_web_app.app_service_linux.name
#   resource_group_name = var.resource_group_name
...
#   thumbprint          = lookup(each.value, "certificate_thumbprint", null) != null ? each.value.certificate_thumbprint : lookup(each.value, "certificate_name", null) != null ? azurerm_app_service_certificate.app_service_certificate[each.key].thumbprint : null
# }
